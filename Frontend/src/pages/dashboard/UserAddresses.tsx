...
const UserAddresses = () => {
  ...
  const fetchAddresses = async () => {
    try {
      ...
      // Adjust according to standard output wrap
  ...
  const handleSetDefault = async (id: string) => {
    try {
      ...
      // Optimistic update for stubbed backends
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         {addresses.map((addr) => (
           <div key={addr._id} className={`relative p-6 rounded-xl border-2 transition-all ${addr.isDefault ? 'border-primary-700 bg-primary-50/20 shadow-sm' : 'border-gray-200 hover:border-primary-300 bg-white'}`}>
              ...
              {addr.isDefault && (
                 <div className="absolute top-0 right-0 bg-primary-700 text-white text-[0.6rem] font-bold uppercase tracking-widest px-3 py-1 rounded-bl-lg rounded-tr-lg">
                    Default
                 </div>
              )}

              <div className="flex justify-between items-start mb-4">
                 <span className="bg-gray-100 text-gray-700 text-[0.65rem] font-bold uppercase tracking-widest px-2.5 py-1 rounded">
                    {addr.type || 'Home'}
                 </span>
                 <div className="flex items-center space-x-2 text-gray-400">
                    <button className="hover:text-primary-700 transition-colors p-1"><Edit2 className="w-4 h-4" /></button>
                    {!addr.isDefault && (
                       <button onClick={() => handleDelete(addr._id!)} className="hover:text-red-600 transition-colors p-1"><Trash2 className="w-4 h-4" /></button>
                    )}
                 </div>
              </div>
              ...
              {!addr.isDefault && (
                 <button onClick={() => handleSetDefault(addr._id!)} className="w-full py-2.5 rounded border border-gray-300 text-sm font-semibold text-gray-600 hover:text-primary-700 hover:border-primary-700 transition-colors uppercase tracking-widest text-[0.7rem]">
                    Set as Default
                 </button>
              )}
           </div>
         ...
         {addresses.length === 0 && (
           <div className="col-span-1 md:col-span-2 p-10 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-500 bg-gray-50">
              ...
              <p className="text-sm">Save your addresses for faster checkout.</p>
           </div>
         )}
      </div>
    </div>
