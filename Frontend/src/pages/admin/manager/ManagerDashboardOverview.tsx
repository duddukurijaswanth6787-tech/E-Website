...
const ManagerDashboardOverview = () => {
  ...
  const quickActions = [
    ...
    { label: 'Analytics Insights', icon: Activity, path: '/manager/analytics', color: 'bg-[var(--admin-card)] text-[var(--admin-text-primary)]' },
  ];
    ...
    <div className="space-y-10 pb-12">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl md:text-4xl font-serif font-bold text-[var(--admin-text-primary)] tracking-tight leading-tight">Manager Command Center</h1>
          <p className="text-stone-500 text-sm mt-1 max-w-md">Real-time production monitoring & bottleneck management.</p>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto">
           <Link to="/manager/workflows" className="flex-1 sm:flex-none px-5 py-2.5 bg-[var(--admin-card)] text-[var(--admin-text-primary)] text-center rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-stone-800 transition-all shadow-xl shadow-stone-900/10">
             Manage Workflow
           </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {quickActions.map((action, i) => (
          <Link 
            key={i} 
            to={action.path}
            className={`p-6 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-3 transition-all hover:scale-[1.05] active:scale-[0.98] ${action.color} shadow-sm border border-[var(--admin-card-border)]`}
          >
            <action.icon size={24} />
           ...
           <div className="space-y-6">
              {[
                ...
                <div key={i} className="space-y-2">
                   ...
                   <div className="h-1.5 w-full bg-[var(--admin-card)] rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
        ...
        <div className="lg:col-span-1 bg-[var(--admin-card)] rounded-[2.5rem] border border-[var(--admin-card-border)] p-8 shadow-sm">
            ...
            <div className="space-y-4">
               {data?.production.workflows.delayed === 0 ? (
                 <div className="py-12 text-center bg-[var(--admin-card)] rounded-3xl border border-dashed border-[var(--admin-card-border)]">
         ...
         {/* Today's Schedule */}
         <div className="lg:col-span-1 bg-[var(--admin-card)] rounded-[2.5rem] border border-[var(--admin-card-border)] p-8 shadow-sm flex flex-col">
            <div className="flex justify-between items-center mb-6">
               <h3 className="text-xl font-serif font-bold text-[var(--admin-text-primary)]">Today's Output</h3>
                 ...
                 data?.production.todayDeliveries.slice(0, 5).map((delivery: any, i: number) => (
                   <div key={i} className="flex items-center justify-between p-3 border-b border-stone-50">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-[var(--admin-card)] flex items-center justify-center text-[10px] font-bold text-stone-400">
                           {i+1}
                         </div>
                         <div>
                            <p className="text-xs font-bold text-[var(--admin-text-primary)]">#{delivery.taskNumber}</p>
      ...
      <div className="bg-[var(--admin-card)] rounded-[2.5rem] border border-[var(--admin-card-border)] p-8 shadow-sm">
         <div className="flex justify-between items-center mb-8">
            <div>
               <h3 className="text-xl font-serif font-bold text-[var(--admin-text-primary)]">Production Load Balancing</h3>
               <p className="text-xs text-stone-400 mt-1 uppercase tracking-widest font-black">Efficiency Indicators</p>
            </div>
         </div>

         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="p-6 bg-[var(--admin-card)] rounded-3xl">
