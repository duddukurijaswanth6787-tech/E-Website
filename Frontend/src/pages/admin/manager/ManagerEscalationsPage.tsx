    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="space-y-4">
        {escalatedTasks.length > 0 ? escalatedTasks.map((task) => (
          <div key={task._id} onClick={() => handleTaskClick(task)} className={`group bg-white border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all cursor-pointer flex flex-col md:flex-row items-center gap-6 ${task.escalationSeverity === EscalationSeverity.BLOCKED ? 'border-red-200 bg-red-50/10' : 'border-stone-100'} ${task.isSlaViolated ? 'border-l-4 border-l-red-600' : ''}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${task.escalationSeverity === EscalationSeverity.BLOCKED ? 'bg-red-600 text-white' : task.escalationSeverity === EscalationSeverity.CUSTOMER_DELAYED ? 'bg-purple-600 text-white' : 'bg-amber-500 text-white'}`}>{task.escalationSeverity}</span>
            ...
            <div className="shrink-0 flex items-center justify-center p-2 bg-stone-50 rounded-full group-hover:bg-amber-500 group-hover:text-white transition-all"><ArrowRight className="w-5 h-5" /></div>
          </div>
        )) : (
