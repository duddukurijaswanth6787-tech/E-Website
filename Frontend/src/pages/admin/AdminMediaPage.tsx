    ...
    <div className="space-y-6 max-w-[100vw] overflow-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Inject static media payloads directly into the AWS/Cloudinary Buckets mapping CDN pathways.</p>
        </div>
        <div className="mt-4 sm:mt-0 flex items-center space-x-3">
           ...
           <button onClick={() => fetchMedia()} className="p-2 text-gray-500 hover:text-primary-700 hover:bg-white rounded border border-gray-200 shadow-sm transition-all" title="Synchronize Bucket">
             <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
           </button>
           <input type="file" ref={fileInputRef} className="hidden" onChange={handleUpload} accept="image/*" />
           <button onClick={() => fileInputRef.current?.click()} disabled={loading} className="flex items-center px-4 py-2 bg-primary-950 text-white text-sm font-bold tracking-widest uppercase rounded shadow hover:bg-primary-800 transition-colors disabled:opacity-50">
              {loading ? (
      ...
      {loading && media.length === 0 ? (
          ...
          <div className="py-24 text-center border-2 border-dashed border-gray-200 rounded-xl bg-white">
             ...
             <p className="text-gray-500 font-serif text-lg">No assets matched your search protocol.</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-6 gap-6">
          {filtered.map((asset, i) => (
            <div key={i} className="bg-white border border-gray-200 rounded-xl overflow-hidden group hover:shadow-xl hover:shadow-primary-950/5 transition-all duration-300 flex flex-col hover:-translate-y-1">
              <div className="h-44 bg-gray-50 relative overflow-hidden flex items-center justify-center p-3">
                ...
                <div className="absolute inset-0 bg-primary-950/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-3">
                   <button onClick={() => copyLink(asset.url)} className="p-2.5 bg-white text-primary-950 hover:bg-blue-50 rounded-full transition-all shadow-lg hover:scale-110" title="Copy Public URL">
                     <Link size={18} />
                   </button>
                   <button onClick={() => toast.error('Object Deletion restricted by environment IAM.')} className="p-2.5 bg-red-600 text-white hover:bg-red-700 rounded-full transition-all shadow-lg hover:scale-110" title="Trash Node">
                     <Trash size={18} />
                   </button>
                </div>
              </div>
              <div className="p-3 border-t border-gray-100 flex-grow flex flex-col justify-between bg-white relative">
