      ...
      <div className="flex flex-wrap items-center gap-4 border-b border-white/5 pb-4">
        {['all', 'active', 'inactive'].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-white/10 text-white border border-white/10' : 'text-gray-500 hover:text-gray-300'}`}
          >
            {f}
          </button>
        ))}
          ...
          >
            ...
            <div className="p-6 space-y-6 flex-grow">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-lg text-white truncate max-w-[200px]">{banner.title || 'Untitled Asset'}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    {banner.section === 'hero' ? <Monitor size={12} className="text-gray-500" /> : <Smartphone size={12} className="text-gray-500" />}
              ...
              <div className="grid grid-cols-2 gap-4 border-y border-white/5 py-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-black text-white">{banner.impressions?.toLocaleString() || 0}</p>
                </div>
                <div className="space-y-1">
