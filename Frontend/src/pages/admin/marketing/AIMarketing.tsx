    ...
    <div className="min-h-screen bg-neutral-950 p-4 sm:p-8 text-white space-y-8 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <GlassCard className="xl:col-span-2">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-bold text-lg text-white">Revenue Forecasting (AI)</h3>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-purple-500" /> <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Predicted</span></div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-white/20" /> <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Actual</span></div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <GlassCard className="border-purple-500/20 bg-purple-500/5">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="font-bold text-lg text-white uppercase tracking-tight">Smart Actions</h3>
            </div>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-purple-500/30 transition-all cursor-pointer group">
                  <div className="flex items-center gap-3 mb-1">
                    <div className="text-purple-400">{rec.icon}</div>
                    <span className="text-xs font-black text-white group-hover:text-purple-400 transition-colors uppercase tracking-widest">{rec.title}</span>
                  </div>
                  <p className="text-[10px] text-gray-500 font-bold ml-7">{rec.desc}</p>
                </div>
              ))}
            </div>
            <button className="w-full py-4 mt-6 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-purple-600/20 transition-all active:scale-95">
      ...
      <GlassCard className="overflow-hidden !p-0">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h3 className="font-bold text-lg text-white">High-Value Customer Predictions</h3>
          <button className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 hover:text-white transition-all">View All Scores &rarr;</button>
        </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-white/5 border-b border-white/5">
              <tr>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest">Engagement Score</th>
                <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest">Predicted CLV</th>
                <th className="px-8 py-5 text-[10px] font-black text-gray-500 uppercase tracking-widest text-right">Probability</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {(predictions as any)?.data?.data?.map((pred: any) => (
                <tr key={pred._id} className="hover:bg-white/[0.02] transition-all">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-xs font-black border border-white/10">
                        {pred.customer?.name?.charAt(0)}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white">{pred.customer?.name}</p>
                        <p className="text-[10px] text-gray-500 font-bold">{pred.customer?.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-xs font-black text-white">{(pred.score * 100).toFixed(0)}/100</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-sm font-black text-emerald-400">₹{pred.type === 'ltv' ? pred.score * 10000 : '4,200'}</td>
                  <td className="px-8 py-6 text-right">
                    <span className="text-xs font-black text-white bg-white/5 px-3 py-1 rounded-lg">High Probability</span>
                  </td>
                </tr>
              ))}
            </tbody>
